...
@rec_eval.register
def rec_eval_primitive_marker(expression: PrimitiveMarker, sketcher: Sketcher,
...
@rec_unroll.register
def rec_unroll_prim_marker(expression:PrimitiveMarker, local_context: LocalContext, sketcher: Sketcher,
...
def eval_primitive_marker(expression: PrimitiveMarker, global_sc: GlobalShaderContext):
    sub_expr = expression.args[0]
...
def eval_stochastic_prim(
    expression: StochasticPrimitive,
    sketcher: Sketcher,
...
# ------------------------------
# Injection helper (now takes 2-logit vector)
# ------------------------------
...
stochastic_dropout_float_code = """
float StochasticPrimitive( float res1, vec2 k )
{
    // Apply softmax on k
    vec2 w = exp(k/0.001);
    w = w / (w.x + w.y);
    res1 = res1 * w.x;// + 1.0 * w.y;
    return res1;   
}
"""
stochastic_dropout_vec_code = Template("""
${type} StochasticPrimitive( ${type} res1, vec2 k )
{
    // Apply softmax on k
    
    res1.x = k.x>k.y? res1.x : 1.0;
    //res1.x = res1.x * w.x;// + 1.0 * w.y;
    return res1;
}
""")

stochastic_dropout_arity_map = {
    ("float", 1): stochastic_dropout_float_code,
    ("vec2", 1): stochastic_dropout_vec_code.substitute(type="vec2"),
    ("vec3", 1): stochastic_dropout_vec_code.substitute(type="vec3"),
    ("vec4", 1): stochastic_dropout_vec_code.substitute(type="vec4"),
}

def stochastic_dropout_factory():
    name = "StochasticPrimitive"
    module = FixedArityShaderModule(name, stochastic_dropout_arity_map)
    return module

SMMap["StochasticPrimitive"] = stochastic_dropout_factory
